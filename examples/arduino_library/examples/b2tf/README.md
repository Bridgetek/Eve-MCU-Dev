# Bridgetek ### EVE LIB NAME ### Touch Screen Test Example

## Touch Screen Test Example

The `touchscreentest` example demonstrates detection of a touch event on a display item, and loading both a custom font and a bitmap into RAM_G. 
...
The application starts up in the file `touchscreentest.ino` which provides initial MCU configuration and the program code for the example. 

The `touchscreentest.ino` code must provide any functions that rely on a platform's operating system, or built-in non-volatile storage mechanism. The required functions store and recall previous touch screen calibration settings:
...
The platform specific functions in `touchscreentest.ino` are called from this routine to store and read touchscreen calibration settings so that the user only needs to perform the action once.
...
| --- | --- |
| [touchscreentest.ino](touchscreentest.ino) | Example source code file |
