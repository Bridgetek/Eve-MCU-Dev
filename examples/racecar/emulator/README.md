# EVE-MCU-Dev RaceCar Example for the EVE Emulator
...
The executable file is normally `build\Debug\racecar_emulator.exe` or  `build\racecar_emulator.exe`. This can be run from the command line. 
...
```console
    cmake --fresh  -G "MinGW Makefiles" -B build -DEVE_DEVICE=EVE_BT817 -DEVE_DISPLAY_RES=EVE_WVGA -S .
```
