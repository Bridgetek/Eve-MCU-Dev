...
The port will support both SPI and QUADSPI hardware drivers in STM32 devices. The HAL calls for these are incompatible and so there are separate C files in the port for SPI and QUADSPI.

To select a single channel SPI interface set the `PLATFORM_STM32_CUBE` macro to a value of `1`, for QUADSPI set the macro to `4`.
If there is no value set to the macro then the single channel SPI will be used by default.

The EVE PD# pin is controlled by software using GPIOs.

### SPI Interface

The common SPI hardware interface on the STM32 is a single channel SPI. There are often multiple of these interfaces, the port to use is configured in STM32CubeMX.
The first interface is called "SPI1" in STM32CubeMX, the second "SPI2" etc. There are a maximum of six interface channels for SPI.
The project generated by STM32CubeMX places a handle for the SPI interface in the `main.c` code.
It is selected in the STM32Cube EVE driver with an `extern` line in the `EVE_MCU_STM32CUBE_SPI.c` file. For example:
```
extern SPI_HandleTypeDef hspi1;
```
The handle is defined in the generated HAL code and drivers.

The STM32Cube EVE driver has an additional macro definition `PLATFORM_STM32_CUBE_CHANNEL` which can select one of the 6 SPI channels.
If the macro is not defined then it will select "SPI1" and use the `hspi1` handle.
